...
int read(char **token)
{
    ...
    nums = strsep(&tmp, "\0"); /* string with the numbers to read */
    snum = strsep(&nums, ","); /* first number */
...
int print(char **token)
{
    ...
    Set *setA = get_set(&set, EOF); /* get the set to print */
    if (!setA)                      /* if the string not represent a valid set */
        return -1;
    while (isspace(*set)) /* skip spaces in end of string */
        set++;
...
int function(char **token, int func)
{
    ...
    set = strsep(&tmp, ",");
    setB = get_set(&set, ','); /* get the 2nd set */
    ...
    set = strsep(&tmp, "\0");
    setC = get_set(&set, EOF); /* get the 3rd set */
...
int run_line()
{
    ...
    function_str = strsep(&ptr, " \t\0"); /* the command text */
    func = get_func(&function_str);   /* int of command */
    if (func < 0)                /* not a valid command */
    {
        if (func == -2)
            return -1;
        if (!ptr) /* if ptr is NULL then we reaced the EOF */
        {
